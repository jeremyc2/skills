...
From the repository root, run this command once. Do not reproduce its steps manually. Report the complete error if it fails.

```bash
bun - <<'BOOTSTRAP_EFFECT'
...
const writeJson = (path, value) => Bun.write(path, `${JSON.stringify(value, null, 2)}\n`);
...
async function ensureLines(path, lines) {
  ...
  if (missing.length) await Bun.write(path, `${text}${text && !text.endsWith("\n") ? "\n" : ""}${missing.join("\n")}\n`);
}
...
// Write the expected Biome shape, then let Biome validate it immediately.
...
// Merge workspace settings without replacing unrelated editor preferences.
